...
func TestSingleSelectNativeFlowRequestsMixedBusinessMetadata(t *testing.T) {
	inner := &waE2E.Message{InteractiveMessage: &waE2E.InteractiveMessage{
		InteractiveMessage: &waE2E.InteractiveMessage_NativeFlowMessage_{NativeFlowMessage: &waE2E.InteractiveMessage_NativeFlowMessage{
			Buttons: []*waE2E.InteractiveMessage_NativeFlowMessage_NativeFlowButton{{Name: proto.String("single_select")}},
		}},
	}}
	wrapped := &waE2E.Message{ViewOnceMessage: &waE2E.FutureProofMessage{Message: inner}}
	biz := buildNativeFlowBizNode(wrapped, 1_700_000_000)
	flow := biz.Content.([]waBinary.Node)[0].Content.([]waBinary.Node)[0]
	if flow.Attrs["name"] != "mixed" {
		t.Fatalf("native-flow name = %q, want mixed", flow.Attrs["name"])
	}
	if buttonName := inner.GetInteractiveMessage().GetNativeFlowMessage().GetButtons()[0].GetName(); buttonName != "single_select" {
		t.Fatalf("button name changed to %q", buttonName)
	}
}
