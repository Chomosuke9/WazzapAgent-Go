...
These remain separate typed model messages/data structures until the provider adapter. The durable history entries are rendered into one final user-role transcript block to keep token cost low; trusted safety instructions must never be concatenated with raw chat content into one untyped system prompt.
