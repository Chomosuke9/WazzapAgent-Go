...
Setelah backup terverifikasi, jalankan binary Part 2. Migration `002_part2_history.sql` diterapkan otomatis dan migration checksum lama diverifikasi. Jangan mengubah migration yang sudah pernah diterapkan.
...
- `HISTORY_WINDOW` membatasi jumlah entry yang dibaca untuk satu invocation.
...
- Di group allowlisted, bot dipicu oleh mention atau reply terhadap response bot yang dapat di-resolve. Group message biasa tetap diabaikan.
...
6. Kirim group text tanpa mention dan tanpa reply. Tidak boleh ada response.
7. Jalankan `/info`; pastikan model, config version, dan status history tampil tanpa prompt, JID, atau secret.
8. Jalankan `/reset` sebagai non-owner; history tidak boleh berubah. Jalankan sebagai owner; follow-up berikutnya harus mulai dengan context kosong.
9. Kirim burst lebih besar dari cap. Semua pesan harus masuk ke batch berurutan tanpa hilang dan tanpa satu batch melewati cap.
10. Putuskan network sebelum native send dimulai; recovery boleh mencoba action yang belum mulai.
11. Putuskan network atau kill process setelah native send mulai tetapi sebelum receipt tersimpan. Setelah lease habis, outcome harus `unknown`; jangan mengirim ulang otomatis.
12. Replay provider event yang sama. Jumlah visible response dan action ID tidak boleh bertambah.
