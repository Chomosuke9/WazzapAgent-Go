...
Transcript mulai dibangun sejak event diterima oleh rewrite ini; tidak ada backfill otomatis dari riwayat provider. Balasan model dan command dicatat sebagai assistant entry, tetapi pesan outgoing manual yang dikirim di luar action outbox belum diimpor. Model context memakai renderer transcript compact (`【id】 HH:MM`, `REPLYING TO`, dan `sender 【senderRef】: text`) agar hemat token. Metadata durable tetap disimpan terstruktur di SQLite.
