...
func TestOpenMigratesLegacySenderRefsWithoutBreakingLIDResolution(t *testing.T) {
	ctx := context.Background()
	path := filepath.Join(t.TempDir(), "app.db")
	store, err := Open(ctx, path)
	if err != nil {
	...
	candidate := testCandidate(t, "legacy-sender-ref", "15550000042@s.whatsapp.net")
	...
	if err := store.Close(); err != nil {
		t.Fatalf("close store before legacy rewrite: %v", err)
	}
	...
	if _, err := db.ExecContext(ctx, "PRAGMA foreign_keys = OFF"); err != nil {
		_ = db.Close()
		t.Fatalf("disable legacy rewrite constraints: %v", err)
	}
	oldRef := claimed.Message.SenderRef.String()
	legacyRef := "u_01234567"
	if _, err := db.ExecContext(ctx, `UPDATE inbound_events SET sender_ref = ?
        WHERE tenant_id = ? AND account_id = ? AND chat_id = ? AND sender_ref = ?`,
		legacyRef, key.TenantID.String(), key.AccountID.String(), key.ChatID.String(), oldRef,
	); err != nil {
		_ = db.Close()
		t.Fatalf("rewrite inbound sender ref: %v", err)
	}
	if _, err := db.ExecContext(ctx, `UPDATE sender_refs SET sender_ref = ?
        WHERE tenant_id = ? AND account_id = ? AND chat_id = ? AND sender_ref = ?`,
		legacyRef, key.TenantID.String(), key.AccountID.String(), key.ChatID.String(), oldRef,
	); err != nil {
		_ = db.Close()
		t.Fatalf("rewrite sender ref mapping: %v", err)
	}
	if err := db.Close(); err != nil {
	...
	ref, err := reopened.Inbound().ResolveSenderRef(ctx, key, candidate.SenderLID)
	if err != nil {
		t.Fatalf("resolve migrated sender ref: %v", err)
	}
	if ref.String() == legacyRef || len(ref.String()) != 6 {
		t.Fatalf("migrated sender ref = %q, want six-character canonical ref", ref.String())
	}
	if _, err := identity.ParseSenderRef(ref.String()); err != nil {
		t.Fatalf("migrated sender ref is invalid: %v", err)
	}
	var inboundRef string
	if err := reopened.db.QueryRowContext(ctx, `SELECT sender_ref FROM inbound_events
        WHERE tenant_id = ? AND account_id = ? AND chat_id = ? AND sender_ref IS NOT NULL`,
		key.TenantID.String(), key.AccountID.String(), key.ChatID.String()).Scan(&inboundRef); err != nil {
		t.Fatalf("read migrated inbound sender ref: %v", err)
	}
	if inboundRef != ref.String() {
		t.Fatalf("inbound sender ref = %q, mapping = %q", inboundRef, ref.String())
	}
}
